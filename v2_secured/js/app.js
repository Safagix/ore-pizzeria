...
const app = {
                    ...
                    sortedProducts.forEach(([name, count]) => {
                        ...
                        visualHTML += `
                            <div class="visual-bar-item">
                                <div class="visual-header"><span>${safeName}</span><span>${count}</span></div>
    ...
    renderFlavors: function () {
        ...
        if (pContainer) {
            if (APP_STATE.products.flavors.length === 0) {
                ...
                pContainer.innerHTML = APP_STATE.products.flavors.map(f => {
                    const safeName = app.escapeHtml(f.name);
                    const safeIngredients = app.escapeHtml(f.ingredients || '');
                    const safeId = app.escapeHtml(f.id);
                    const safeImg = app.escapeHtml(f.img || '');

                    return `
                    <div class="flavor-card" id="card-${safeId}"
                         data-id="${safeId}"
                         onclick="app.selectFlavor(this.dataset.id)">
                        ...
                        ${f.ingredients ? `<p style="color: #bbb; font-size: 0.65rem; margin: 4px 0; line-height: 1.2;">${safeIngredients}</p>` : ''}
        ...
        if (dContainer) {
            dContainer.innerHTML = APP_STATE.products.drinks.map(d => {
                ...
                const safeId = app.escapeHtml(d.id);
                const safeImg = app.escapeHtml(d.img || '');

                return `
                <div class="flavor-card"
                     data-id="${safeId}"
                     onclick="app.addDrink(this.dataset.id)">
    ...
    renderCart: function () {
        ...
        c.innerHTML = APP_STATE.cart.map((item, i) => {
            total += item.price;
            const safeName = app.escapeHtml(item.name);
            const safeNotes = app.escapeHtml(item.notes || '');
            return `
            <div class="cart-item">
    ...
    renderChef: function () {
        ...
        container.innerHTML = ordersToShow.map(o => {
            ...
            let itemsHtml = items.map(i => {
                ...
                const safeDisplayName = app.escapeHtml(displayName);
                const safeNotes = app.escapeHtml(i.notes || '');

                return `<li>${safeDisplayName} ${i.notes ? `<br><small style='color:#f57c00'>(${safeNotes})</small>` : ''}</li>`;
            }).join('');

            const safeCustomer = app.escapeHtml(o.customer || 'S/N');
            const safeTime = app.escapeHtml(o.timestamp);
