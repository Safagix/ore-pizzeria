...
const app = {
    ...
    login: async function () {
        ...
        // Security Context: Ensure Web Crypto is available (requires secure context)
        if (!window.crypto || !window.crypto.subtle) {
            return alert("Error de seguridad: El navegador no permite criptografía segura. Use HTTPS o localhost.");
        }

        const hashedPin = await this.hashPin(pin);

        if (hashedPin !== this._AUTH_HASHES[role]) {
